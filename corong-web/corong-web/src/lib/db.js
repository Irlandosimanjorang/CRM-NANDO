...
// ---- ADVISOR (hasil analisis tersimpan) ----
export async function getAdvisor() {
  const { data } = await supabase.from("advisor_runs").select("*").maybeSingle();
  return data || { ran_at: "", recs: [] };
}
export async function saveAdvisor(ran_at, recs) {
  const uid = (await supabase.auth.getUser()).data.user.id;
  const { error } = await supabase.from("advisor_runs").upsert({ user_id: uid, ran_at, recs, updated_at: new Date().toISOString() });
  if (error) throw error;
}
