...
import { CalendarCheck, Plus, Search, Save, X } from "lucide-react";
import * as db from "../lib/db";
import { typeBadge, fmtDate, todayISO } from "../lib/helpers";
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 flex items-start justify-center p-4 z-50 overflow-y-auto" onClick={onClose}>
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl my-8 p-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4"><h2 className="font-bold text-lg flex items-center gap-2"><CalendarCheck size={18} className="text-amber-500" /> Tambah Visit</h2><button onClick={onClose} className="text-slate-400 hover:text-slate-700"><X size={20} /></button></div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div className="flex items-center gap-2"><CalendarCheck size={20} className="text-amber-500" /><h1 className="text-2xl font-bold tracking-tight">Visit</h1><span className="text-sm text-slate-400">({visits.length})</span></div>
        <button onClick={() => setAdd(true)} className="flex items-center gap-1.5 bg-amber-600 hover:bg-amber-700 text-white text-sm px-3 py-2 rounded-xl font-medium shadow-sm shadow-amber-600/20"><Plus size={15} /> Tambah visit</button>
      </div>
      {visits.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-3"><div className="text-xs text-slate-400 mb-1 flex items-center gap-1"><CalendarCheck size={13} /> Akan datang</div><div className="font-mono font-bold text-2xl text-amber-600">{upcoming.length}</div></div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {visits.map((c) => { const past = c.visit_date < todayISO(); const today = c.visit_date === todayISO(); const meet = c.visit_meet || c.key_person; return (
                  <tr key={c.id} className={`border-t border-slate-100 hover:bg-amber-50/40 cursor-pointer ${past ? "opacity-50" : ""}`} onClick={() => onEdit(c)}>
                    ...
                    <td className="px-3 py-2 text-xs"><span className={today ? "text-amber-600 font-medium" : "text-slate-600"}>{fmtDate(c.visit_date)}{today && " · hari ini"}</span></td>
