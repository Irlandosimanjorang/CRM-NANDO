            ...
            {pendingApprovals.map((r) => (
              <div key={r.id} className="flex items-center justify-between gap-2 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2">
                <div className="text-xs text-slate-700">
                  ...
                  {r.action_type === "delete_lead" ? <>hapus lead <b>{r.payload?.lead_name || "ini"}</b></> : r.action_type === "delete_competitor" ? <>hapus kompetitor <b>{r.payload?.competitor_name || "ini"}</b></> : "export data leads"}
                </div>
