  ...
  return (
    <>
      ...
      <Section title="3. Akun Pengguna">
        <p>Anda bertanggung jawab menjaga kerahasiaan kredensial akun (email, password, dan kode 2FA jika diaktifkan). Nexto tidak bertanggung jawab atas kerugian akibat akses tidak sah ke akun Anda yang disebabkan kelalaian menjaga kredensial tersebut.</p>
        <p>Anda wajib memberikan informasi yang akurat saat mendaftar dan menjaga informasi tersebut tetap terkini.</p>
      </Section>
      ...
      <Section title="4. Paket Berlangganan & Pembayaran">
        ...
        <p>Perpanjangan langganan bersifat otomatis kecuali dibatalkan sebelum periode tagihan berikutnya. Anda dapat membatalkan atau upgrade/downgrade paket kapan saja melalui tab Pengaturan.</p>
        <p>Sebagai aturan umum, pembayaran yang sudah dilakukan tidak dapat dikembalikan (non-refundable), termasuk untuk sisa periode berlangganan yang belum terpakai setelah pembatalan. Pengecualian: kami akan memberikan pengembalian dana penuh atau sebagian jika (a) Anda tertagih lebih dari satu kali untuk periode langganan yang sama akibat kesalahan sistem/pembayaran (double-charge), atau (b) Layanan berbayar yang Anda langgan tidak dapat diakses sama sekali (bukan gangguan sebagian/sementara) selama lebih dari 72 jam berturut-turut akibat kesalahan pada sisi kami. Permintaan pengembalian dana diajukan melalui WhatsApp support paling lambat 14 hari sejak kejadian.</p>
      ...
      <Section title="5. Fitur Kecerdasan Buatan (AI)">
        <p>Nexto menyediakan fitur berbasis AI (analisis lead, draft pesan follow-up, rekomendasi harian, asisten chat, dan sejenisnya) yang memproses data yang Anda masukkan ke dalam sistem. Rekomendasi dan konten yang dihasilkan AI bersifat <b>bantuan pengambilan keputusan</b>, bukan nasihat profesional (hukum, keuangan, atau bisnis) dan bisa saja tidak akurat.</p>
        <p>Anda tetap bertanggung jawab penuh atas keputusan bisnis dan komunikasi yang dikirim ke pelanggan/lead Anda, termasuk yang dibantu disusun oleh AI.</p>
      </Section>

      <Section title="6. Kepemilikan Data">
        <p>Semua data yang Anda masukkan ke Nexto (data lead, catatan progress, data kompetitor, dsb.) tetap menjadi milik Anda. Nexto tidak mengklaim kepemilikan atas data tersebut dan tidak menjual data Anda ke pihak ketiga manapun.</p>
      </Section>

      <Section title="7. Penggunaan yang Dilarang">
        <p>Anda setuju untuk tidak: (a) menggunakan Layanan untuk aktivitas ilegal; (b) mencoba mengakses sistem atau data milik pengguna lain tanpa izin; (c) melakukan reverse engineering, scraping otomatis, atau membebani sistem secara berlebihan (abuse); (d) menggunakan fitur AI untuk menghasilkan konten yang menyesatkan, melecehkan, atau melanggar hukum.</p>
      </Section>
      ...
      <Section title="10. Penghentian Layanan">
        <p>Anda dapat menghentikan penggunaan Layanan kapan saja. Nexto berhak menangguhkan atau menghentikan akun yang melanggar Ketentuan Layanan ini, dengan pemberitahuan bila memungkinkan.</p>
      </Section>
      ...
      <Section title="12. Hukum yang Berlaku & Penyelesaian Sengketa">
        <p>Ketentuan ini diatur dan ditafsirkan sesuai hukum Republik Indonesia. Setiap perselisihan yang timbul dari penggunaan Layanan akan diupayakan diselesaikan terlebih dahulu secara musyawarah untuk mufakat antara Anda dan Nexto. Apabila tidak tercapai kesepakatan dalam waktu 30 hari, perselisihan akan diselesaikan melalui Pengadilan Negeri sesuai domisili hukum penyelenggara Layanan di Indonesia.</p>
      </Section>
  ...
  return (
    <>
      ...
      <Section title="2. Bagaimana Data Digunakan">
        <p>Data Anda digunakan untuk: menyediakan fungsi inti CRM, menjalankan fitur AI (analisis, draft pesan, rekomendasi), mengirim notifikasi/email terkait akun, dan meningkatkan keandalan Layanan.</p>
      </Section>

      <Section title="3. Dasar Hukum Pemrosesan Data">
        <p>Sesuai Undang-Undang Nomor 27 Tahun 2022 tentang Pelindungan Data Pribadi ("UU PDP"), kami memproses data pribadi Anda berdasarkan: (a) <b>persetujuan Anda</b> saat mendaftar dan menyetujui Kebijakan Privasi ini; (b) <b>pelaksanaan kontrak</b> layanan berlangganan antara Anda dan Nexto; dan (c) <b>kepentingan sah</b> kami untuk menjaga keamanan sistem dan mencegah penyalahgunaan Layanan.</p>
      </Section>

      <Section title="4. Pemrosesan oleh AI Pihak Ketiga (Penting)">
        <p>Untuk menjalankan fitur AI, sebagian data CRM Anda (nama lead, catatan progress, konteks bisnis relevan) dikirim ke penyedia model AI pihak ketiga: <b>Anthropic (Claude)</b> dan <b>OpenAI</b> (untuk transkripsi suara dan embedding pencarian semantik). Data ini diproses sesuai kebijakan privasi masing-masing penyedia dan tidak digunakan oleh mereka untuk melatih model AI mereka berdasarkan perjanjian API komersial standar.</p>
        <p>Kami hanya mengirim data yang relevan untuk menghasilkan output yang diminta (misalnya draft follow-up untuk satu lead), bukan seluruh database Anda sekaligus.</p>
      </Section>
      ...
      <Section title="6. Transfer Data ke Luar Negeri">
        <p>Sebagian penyedia layanan pada Bagian 4 dan 5 (termasuk Anthropic, OpenAI, Supabase, dan Vercel) memproses dan/atau menyimpan data pada server yang berlokasi di luar wilayah Indonesia (Amerika Serikat dan/atau negara lain). Dengan menggunakan Layanan, Anda memahami dan menyetujui bahwa data pribadi Anda dapat ditransfer dan diproses di luar Indonesia. Kami hanya bekerja sama dengan penyedia yang memiliki kebijakan privasi dan standar keamanan data yang memadai untuk perjanjian pemrosesan data komersial.</p>
      </Section>

      <Section title="7. Keamanan Data">
        <p>Kami menerapkan Row Level Security (RLS) sehingga data organisasi Anda terisolasi dari organisasi lain, autentikasi dua faktor (2FA) opsional, dan pencatatan audit log untuk aktivitas sensitif (hapus/ubah data penting). Namun, tidak ada sistem yang 100% bebas risiko - Anda tetap disarankan menjaga kerahasiaan kredensial akun.</p>
      </Section>

      <Section title="8. Pemberitahuan Insiden Keamanan Data">
        <p>Apabila terjadi insiden yang mengakibatkan kegagalan pelindungan data pribadi Anda (kebocoran data), kami akan menyampaikan pemberitahuan tertulis kepada Anda selambat-lambatnya 3x24 jam sejak insiden diketahui, sesuai dengan yang diamanatkan UU PDP, termasuk sifat insiden dan langkah mitigasi yang kami lakukan. Kami juga akan melaporkan insiden tersebut kepada otoritas pengawas Pelindungan Data Pribadi sesuai ketentuan yang berlaku.</p>
      </Section>

      <Section title="9. Hak Anda Atas Data">
        <p>Sesuai UU PDP, Anda berhak: (a) mengakses dan mendapatkan salinan data Anda (fitur export tersedia di tab Pengaturan); (b) meminta koreksi data yang tidak akurat atau tidak lengkap; (c) meminta penghapusan dan/atau pemusnahan data pribadi Anda (hubungi support); (d) menarik kembali persetujuan yang pernah diberikan; (e) mengajukan keberatan atas keputusan yang semata-mata didasarkan pada pemrosesan otomatis (termasuk rekomendasi AI) yang menimbulkan akibat hukum atau berdampak signifikan bagi Anda; (f) menggugat dan menerima ganti rugi atas pelanggaran pemrosesan data pribadi Anda sesuai ketentuan perundang-undangan; dan (g) mengajukan pengaduan kepada otoritas pengawas Pelindungan Data Pribadi yang berwenang di Indonesia. Beberapa data dapat kami simpan lebih lama jika diwajibkan hukum (misalnya catatan transaksi pembayaran).</p>
      </Section>

      <Section title="10. Retensi Data">
        <p>Data disimpan selama akun Anda aktif. Data yang dihapus (lead, kompetitor) masuk ke recycle bin untuk periode tertentu sebelum terhapus permanen. Jika akun dihentikan/dihapus, data akan dihapus dari sistem produksi dalam waktu wajar, kecuali disyaratkan hukum untuk disimpan lebih lama.</p>
      </Section>

      <Section title="11. Cookie & Local Storage">
        <p>Kami menggunakan penyimpanan lokal browser (local storage) untuk menyimpan preferensi tampilan dan status sesi - bukan untuk melacak aktivitas Anda di situs lain atau untuk periklanan.</p>
      </Section>
      ...
      <Section title="13. Identitas Pengendali Data">
        <p>Pengendali data pribadi (Data Controller) untuk Layanan ini adalah penyelenggara Nexto, yang dapat dihubungi melalui kontak pada Bagian 14 di bawah untuk segala pertanyaan atau permohonan terkait data pribadi Anda sebagaimana diatur UU PDP.</p>
      </Section>
      ...
      <Section title="15. Kontak">
        <p>Pertanyaan mengenai privasi dan data Anda, termasuk permohonan terkait hak-hak pada Bagian 9, dapat disampaikan melalui WhatsApp support Nexto (lihat tombol di bawah).</p>
      </Section>
    </>
  );
}
