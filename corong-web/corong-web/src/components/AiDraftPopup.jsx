...
// === BUG FIX (5 Sep 2026) ===
// 1. Draft (WA/Email) sekarang disimpen di server (tabel ai_drafts) SEBELUM
//    response dibalikin - jadi walau tab/koneksi HP mati di tengah generate,
//    hasilnya tetep tersimpen & bisa diambil ulang tanpa generate baru.
// 2. Popup INI SENDIRI sekarang juga bisa "auto-restore" abis reload total -
//    dicek lewat rect=null (dipanggil tanpa event klik tombol, jadi gak ada
//    posisi buat nempel) - dalam kondisi ini popup ditampilin di TENGAH layar,
//    bukan nempel ke tombol yang udah gak ada lagi konteksnya.
...
export default function AiDraftPopup({ lead, rect, onClose, onSent, initialChannel }) {
  ...
  const runDraft = async (ch) => {
    ...
    try {
      ...
      // komentar BUG FIX di atas). Field `cached` cuma buat info di UI.
      const res = await db.draftFollowup(lead.id, ch);
      setWasCached(!!res.cached);
  ...
  // Kalau dibuka dari tombol "Handle Now" (bawa initialChannel) ATAU
  // di-restore otomatis abis reload dengan channel yang udah tersimpen,
  // langsung auto-generate/ambil draft-nya tanpa nunggu user klik channel dulu.
  useEffect(() => {
  ...
  const POPUP_W = 336;
  const POPUP_H = 420;
  // rect null (di-restore otomatis, gak ada tombol yang diklik) -> nampilin
  // di TENGAH layar. Ada rect (diklik manual) -> nempel di deket tombolnya
  // kayak biasa.
  const left = rect
    ? Math.min(Math.max(rect.left - 40, 12), window.innerWidth - POPUP_W - 12)
    : Math.max(12, (window.innerWidth - POPUP_W) / 2);
  const top = rect
